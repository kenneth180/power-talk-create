  ...
  const sendMessage = useCallback(
    async (content: string, imageBase64?: string, styleId?: string) => {
      ...
      if (imageBase64) {
        ...
        const detectedUrl = extractWebUrl(content);
        let assistantSoFar = "";
        const assistantId = crypto.randomUUID();

        if (detectedUrl) {
          // Show web preview immediately with a message
          const previewMsg: Message = {
            id: assistantId,
            role: "assistant",
            content: `🌐 Opening **${detectedUrl}** for you!`,
            timestamp: new Date(),
            webUrl: detectedUrl,
          };
          setMessages((prev) => ({
            ...prev,
            [chatId!]: [...(prev[chatId!] || []), previewMsg],
          }));
          setIsLoading(false);
          scrollToBottom();
